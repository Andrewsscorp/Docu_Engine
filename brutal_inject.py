...
edit_modal_html = '''
    ...
    <div x-show="showEditModal" x-transition.opacity style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center p-4 backdrop-blur-sm bg-gray-900/50" 
        ...
        }" @change-theme="currentBg = .detail.bg; currentText = .detail.text">
...
if "El Modal de Edición de Etiquetas" not in content:
    idx = content.find("<!-- END OF TAGS SECTION -->")
    if idx == -1:
        # Just find the closing form of the creation modal, and append there.
        # It's inside the tags section.
        form_end = content.find('''</form>\n        </div>\n    </div>\n\n</section>''')
        if form_end != -1:
            idx2 = form_end + len('''</form>\n        </div>\n    </div>''')
