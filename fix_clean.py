...
# Instead of patching the broken file, let's take the pristine old file and apply the 3 changes:
# 1. Update x-data
...
# 2. Add filter to Documentos PDF list (only the FIRST occurrence of this header)
...
original = original.replace(old_header, new_header, 1) # Only replace the FIRST ONE!
...
original = original.replace(old_tr, new_tr)

# 4. Fix Opciones column (replace drawer with visor forense)
old_eye = '<button @click.stop="$dispatch(\'open-drawer\', \'{{ doc.id }}\')" class="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors tooltip" title="Ver Documento">'
new_eye = '<button @click.stop="visorPdfUrl = \'/api/v1/agn/documentos/{{ doc.id }}/ver_forense\'; visorAbierto = true;" class="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors tooltip" title="Visor Forense">'
original = original.replace(old_eye, new_eye)

# 5. Add Forensic Download button
old_dl = '<button @click.stop="window.location.href = \'/api/v1/documents/{{ doc.id }}/download\'" class="p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors tooltip" title="Descargar PDF">'
new_dl = '<button @click.stop="Swal.fire({toast:true, position:\'top-end\', icon:\'info\', title:\'Descarga Forense Iniciada...\', showConfirmButton:false, timer:2000}); window.location.href = \'/api/v1/agn/documentos/{{ doc.id }}/descargar_forense\';" class="p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors tooltip" title="Descarga Física">'
original = original.replace(old_dl, new_dl)

# 6. Append Visor PDF Modal at the end of the file, just before </div> \n <script>
visor_modal = """
...
"""
original = original.replace('    <script>', visor_modal + '\n    <script>')
