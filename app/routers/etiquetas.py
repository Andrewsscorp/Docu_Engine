...
def get_row_html(et, uso_count):
    ...
    kebab_menu += """
                ...
                <li>
                    <button @click.prevent="open=false; Swal.fire('Tróximamente', 'El módulo de permisos RBAC estará disponible en la versión 2.0', 'info')" class="w-full text-left px-4 py-2 hover:bg-slate-50 flex items-center">
    ...
    if not et.es_sistema:
        kebab_menu += f"""
                    <button hx-delete="/api/v1/etiquetas/{et.id_etiqueta}" hx-confirm="ßSeguro que deseas desactivar esta etiqueta?" hx-target="closest tr" hx-swap="outerHTML" class="w-full text-left px-4 py-2 hover:bg-red-50 text-red-600 flex items-center">
                        <span class="mr-2">🔮</span> Desactivar
                    </button>
        """
    else:
        kebab_menu += f"""
...
async def list_etiquetas(request: Request, categoria: str = Query("Todos"), db: AsyncSession = Depends(get_db_session)):
    try:
        ...
        if not html_out:
            return HTMLResponse("<tr><td colspan='5' class='py-8 text-center text-gray-500'>No hay etiquetas en esta categoria</td></tr>")
...
async def update_etiqueta(
    ...
    try:
        ...
        if uso_count > 0:
            ...
            detalles = json.dumps({
                "etiqueta_id": id,
                "intento_cambio_nombre": nombre,
                "razon_bloqueo": f("En uso por {uso_count} documentos"
            })
            ...
            # Htmx Trigger for the warning alert
            response = HTMLResponse(content="<div class='text-red-600'>Error: Etiqueta Inmutable</div>") 
